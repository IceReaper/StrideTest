...
namespace StrideTest.Assets.Models
{
	...
	public static class GltfUtils
	{
		...
		/// <returns></returns>
		public static Matrix ConvertNumerics(Matrix4x4 mat)
		{
			return new(
				mat.M11,
				mat.M12,
		...
		/// <param name="v"></param>
		/// <returns></returns>
		public static Quaternion ConvertNumerics(System.Numerics.Quaternion v)
		{
		...
		/// <param name="v"></param>
		/// <returns></returns>
		public static Vector4 ConvertNumerics(System.Numerics.Vector4 v)
		{
			return new(v.X, v.Y, v.Z, v.W);
		}
		...
		/// <param name="v"></param>
		/// <returns></returns>
		public static Vector3 ConvertNumerics(System.Numerics.Vector3 v)
		{
		...
		/// <param name="v"></param>
		/// <returns></returns>
		public static Vector2 ConvertNumerics(System.Numerics.Vector2 v)
		{
			return new(v.X, v.Y);
		}
		...
		{
			return new() { GltfUtils.ConvertNumerics(value.Item1), GltfUtils.ConvertNumerics(value.Item2), GltfUtils.ConvertNumerics(value.Item3) };
		}
		...
		/// <returns></returns>
		public static (VertexElement, int) ConvertVertexElement(KeyValuePair<string, Accessor> accessor, int offset)
		{
			...
			return (accessor.Key, accessor.Value.Format.ByteSize) switch
			{
				...
				("COLOR_0", 8) => (new(VertexElementUsage.Color, 0, PixelFormat.R16G16B16A16_Float, offset), 8),
				...
				("JOINTS_0", 8) => (new(VertexElementUsage.BlendIndices, 0, PixelFormat.R16G16B16A16_UInt, offset), 8),
				("JOINTS_0", 4) => (new(VertexElementUsage.BlendIndices, 0, PixelFormat.R8G8B8A8_UInt, offset), 4),
				("WEIGHTS_0", 4) => (new(VertexElementUsage.BlendWeight, 0, PixelFormat.R8G8B8A8_UInt, offset), 4),
				("WEIGHTS_0", 8) => (new(VertexElementUsage.BlendWeight, 0, PixelFormat.R16G16B16A16_Float, offset), 8),
				("WEIGHTS_0", 16) => (new(VertexElementUsage.BlendWeight, 0, PixelFormat.R32G32B32A32_Float, offset), 16),
		...
		/// <returns></returns>
		public static List<string> GenerateTextureFullPaths(ModelRoot root, string sourcePath)
		{
			...
			foreach (var mat in root.LogicalMaterials)
			{
				foreach (var chan in mat.Channels)
				{
					if (chan.Texture != null && !chan.HasDefaultContent)
					{
						...
						{
							var textureName = gltfImg.Name
								?? Path.GetFileNameWithoutExtension(sourcePath) + "_" + (mat.Name ?? mat.LogicalIndex.ToString()) + "_" + chan.Key;

							result.Add(Path.Join(Path.GetFullPath(sourcePath), textureName + "." + gltfImg.Content.FileExtension));
						}
						else
		...
		/// <returns></returns>
		public static ComputeTextureColor GenerateTextureColor(
			string sourceTextureFile,
			TextureCoordinate textureUVSetIndex,
		...
		)
		{
			...
			var currentTexture =
				new ComputeTextureColor(texture, textureUVSetIndex, uvScaling, Vector2.Zero) { AddressModeU = addressModeU, AddressModeV = addressModeV };
		...
		/// <param name="vfsOutputPath"></param>
		/// <returns></returns>
		public static ComputeTextureScalar GenerateTextureScalar(
			string sourceTextureFile,
			TextureCoordinate textureUVSetIndex,
			Vector2 textureUVscaling,
			TextureAddressMode addressModeU = TextureAddressMode.Wrap,
		...
		)
		{
			...
			var currentTexture =
				new ComputeTextureScalar(texture, textureUVSetIndex, uvScaling, Vector2.Zero) { AddressModeU = addressModeU, AddressModeV = addressModeV };
